...
##エラー対応で手作業で運転した場合は、Flourishのリンク先のURLをgithub上のcsvに切り替えなおす必要があります
#詳細はissue
...
#longフォーマットに転換
wbgt = wbgt.set_index(['Date','Time','dt_popup','date_dt']).rename_axis('amdno',axis=1).stack().rename('wbgt').reset_index()
...
#lat longを整える
#Flourishではlongになっていたので
locations[['lat','long']] = locations[['lat','lon']].applymap(lambda x: x[0] + x[1]/60)

#地点情報を最新データに追加
##how='left'
wbgt_latest = wbgt_latest.merge(locations[['amdno','lat','long','kjName']], on='amdno', how='left')
